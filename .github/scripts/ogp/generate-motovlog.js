 ...
 *   images/favicons/motovlog-favicon.png          192x192（フルブリードの円）
 *   images/favicons/motovlog-apple-touch-icon.png 180x180（角まで埋めた四角）
 *
 * カードの色・書体・グラデーションは motovlog/index.html のヒーローと同じ値。
 * ページ側を触ったらここも合わせること。バッジは motovlog-favicon.svg が正本で、
 * PNG 2枚はそれを焼いたもの（SVG を直せば両方この場で作り直せる）。
 *
...
const MARK = 'images/favicons/motovlog-favicon.svg';
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .mark{width:62px;height:62px;flex:0 0 auto}
...
img.src = ${JSON.stringify(dataUri(MARK))};
<\/script></body></html>`;
}
...
(async () => {
  ...
  try {
    ...
    for (const [name, size, pad, bg] of [
      ['motovlog-favicon', 192, 0, ''],
      ['motovlog-apple-touch-icon', 180, 0.08, '#11141b'],
    ]) {
